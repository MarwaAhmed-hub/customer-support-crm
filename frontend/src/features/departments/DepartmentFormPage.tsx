...
/**
 * Serves both `/departments/new` (no `:id`) and `/departments/:id/edit` (`:id` present) — mode is
 * read from the route, matching `RoleFormPage`'s pattern.
 */
export function DepartmentFormPage() {
  ...
  useEffect(() => {
    ...
    departmentsApi
      ...
      .catch(() => {
        if (!cancelled) setFormError("Could not load this department. Please try again.");
      })
      .finally(() => {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      if (isEdit && id !== undefined) {
        await departmentsApi.updateDepartment(id, { name: name.trim(), code: codePayload, isActive });
      } else {
        await departmentsApi.createDepartment({ name: name.trim(), code: codePayload });
      }
      navigate("/departments", { replace: true });
    } catch (caught: unknown) {
      if (axios.isAxiosError(caught) && caught.response?.status === 409) {
        const errorCode: unknown = caught.response.data?.error;
        if (errorCode === "duplicate_department_code") {
          setFieldErrors((current) => ({ ...current, code: "A department with this code already exists." }));
        } else {
          setFieldErrors((current) => ({ ...current, name: "A department with this name already exists." }));
        }
      } else {
    ...
    <Box sx={{ maxWidth: 480 }}>
      ...
      <Paper component="form" onSubmit={handleSubmit} noValidate variant="outlined" sx={{ p: 3 }}>
        ...
        <Box sx={{ display: "flex", gap: 1.5, mt: 3 }}>
          ...
          <Button variant="text" onClick={() => navigate("/departments")}>
            Cancel
          </Button>
        </Box>
      </Paper>
