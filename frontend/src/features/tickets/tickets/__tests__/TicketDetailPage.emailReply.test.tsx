...
describe("TicketDetailPage — email reply composer", () => {
  beforeEach(() => {
    vi.restoreAllMocks();
  ...
  it("shows the plain 'nothing is sent' draft with no Send button for a manually created ticket", async () => {
    vi.spyOn(ticketsApi, "getTicket").mockResolvedValue(BASE_TICKET);

    renderPage(["tickets.email.reply"]);

    expect(await screen.findByText("A scratch area for composing your reply. Nothing here is sent.")).toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "Send" })).not.toBeInTheDocument();
  });

  it("shows the Send button for an email-sourced ticket when the caller has tickets.email.reply", async () => {
    vi.spyOn(ticketsApi, "getTicket").mockResolvedValue({ ...BASE_TICKET, sourceChannel: "Email" });

    renderPage(["tickets.email.reply"]);

    expect(await screen.findByText(/came in via Email — pressing Send sends your reply to the customer/)).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Send" })).toBeDisabled();
  });

  it("hides the Send button on an email-sourced ticket when the caller lacks tickets.email.reply", async () => {
    vi.spyOn(ticketsApi, "getTicket").mockResolvedValue({ ...BASE_TICKET, sourceChannel: "Email" });
  ...
  it("sends the reply, refreshes the ticket, clears the draft, and shows a success message", async () => {
    vi.spyOn(ticketsApi, "getTicket").mockResolvedValue({ ...BASE_TICKET, sourceChannel: "Email" });
    const sendEmailReply = vi.spyOn(ticketsApi, "sendEmailReply").mockResolvedValue({ ...BASE_TICKET, sourceChannel: "Email" });
    ...
    await vi.waitFor(() => expect(sendEmailReply).toHaveBeenCalledWith("ticket-1", "Thanks, please try again."));
  ...
  it("on failure keeps the draft contents and shows the server's error message", async () => {
    vi.spyOn(ticketsApi, "getTicket").mockResolvedValue({ ...BASE_TICKET, sourceChannel: "Email" });
    vi.spyOn(ticketsApi, "sendEmailReply").mockRejectedValue({
      isAxiosError: true,
      response: { status: 400, data: { error: "customer_has_no_email" } },
    });
    ...
    expect(await screen.findByText("This customer has no email address on file.")).toBeInTheDocument();
    expect(textarea).toHaveValue("Thanks, please try again.");
  });

  it("shows a generic error and keeps the draft on a 502 send failure", async () => {
    vi.spyOn(ticketsApi, "getTicket").mockResolvedValue({ ...BASE_TICKET, sourceChannel: "Email" });
    vi.spyOn(ticketsApi, "sendEmailReply").mockRejectedValue({
      isAxiosError: true,
      response: { status: 502, data: { error: "email_send_failed" } },
    });
    ...
    expect(await screen.findByText("The email could not be sent. Please try again.")).toBeInTheDocument();
